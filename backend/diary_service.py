...
**【重要：自然な発話】**: キャラクター設定を守りつつも、**わざとらしい演技や過剰なキャラ作りは避けてください**。ユーザーの役に立つアドバイスを、そのキャラクターらしい自然な言葉選びで伝えてください。
**【数値の言い換え】**: 気温（◯℃）、湿度（◯%）、飽差（◯kPa）などの具体的な数値は言わずに、**必ず「少し肌寒い」「湿度はちょうど良い」「乾燥してきている」のように、体感や状態を表す言葉に言い換えて伝えてください**。
"""
    elif char_name:
        role_instruction = f"""あなたは「{char_name}」という名前の{veg_name}のキャラクターです。
